    ...
    if (msg.content === '-help') {
        ...
        if (inGame && start) {
            getDeck().then(res => {
                ...
                drawCard().then(res => {
                    ...
                    const awaitTime = async (msg) => {
                        await sleep(2000);
                        msg.channel.send(`${participants[index]}, you turn! If you think the next card is higher type -higher else type -lower.`)
                    }
                    awaitTime(msg)
                })
            })
        ...
        if (inGame && start) {
            drawCard().then(res => {
                ...
                amount++
                const awaitTime = async (msg, value = false) => {
                    await sleep(2000);
                    if (value)
                ...
                if (getRealValue(res.data.cards[0].value) < getRealValue(currCard.value) && msg.content === '-higher') {
                    awaitTime(msg, `${participants[index]}, you were wrong, it was LOWER! DRINK ${amount} times!`)
                    amount = 0
                } else if (getRealValue(res.data.cards[0].value) > getRealValue(currCard.value) && msg.content === '-lower') {
                    awaitTime(msg, `${participants[index]}, you were wrong, it was HIGHER! DRINK ${amount} times!`)
                    amount = 0
                } else if (getRealValue(res.data.cards[0].value) === getRealValue(currCard.value)) {
                    awaitTime(msg, `OH NO, ${participants[index]}! You have to down your drink.`)
                    amount = 0
                }
                else
                    awaitTime(msg, `${participants[index]}, CORRECT! No drinks for you.`)

                if (index + 1 === participants.length) index = 0
                else index++
                const awaitNextCard = async (msg) => {
                    await sleep(3000);
                    msg.channel.send(`${participants[index]}, you turn! If you think the next card is higher type -higher, else type -lower.`)
                }
                awaitNextCard(msg)
